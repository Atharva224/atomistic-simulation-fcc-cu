...
for filename in os.listdir(folder_path):
    if filename.startswith("log_bcc_") and filename.endswith(".lammps"):
        # Extract the lattice constant from the filename
        match = re.search(r"log_bcc_(\d+\.\d+)\.lammps", filename)
        if match:
            ...
            with open(full_path, 'r') as file:
                ...
                for i, line in enumerate(lines):
                    if "Step" in line and "E_pair" in line:
                        ...
                        if i + 1 < len(lines):
                            bcc_data_line = lines[i + 1].strip()
                            values = bcc_data_line.split()
                            try:
...
for a, e in zip(lattice_constants, e_pair_values):
    print(f"Lattice constant: {a:.2f} Å -> E_pair: {e:.6f} eV")

# ✅ Save results to file
output_file = os.path.join(folder_path, "lattice_vs_epair.txt")
